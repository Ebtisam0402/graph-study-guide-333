...
public class Practice {
  ...
  private static void sortedReachable(Vertex<Integer> current, Set<Vertex<Integer>> visited, List<Integer> values){
  ...
  private static void sortedReachable(Map<Integer, Set<Integer>> graph, int current, Set<Integer>visited, List<Integer> values){
    if(visited.contains(current)) return;
  ...
  public static <T> boolean twoWay(Vertex<T> v1, Vertex<T> v2) {
    ...
    return canReach(v1, v2, new HashSet<>()) && 
           canReach(v2, v1, new HashSet<>());
  }
  ...
  public static List<int[]> nextMoves(char[][] board, int[] current, int[][] directions) {
    List<int[]> moves = new ArrayList<>();
     ...
     for(int[] direction: directions){
      int newR = currentRow + direction[0];
      int newC = currentCol + direction[1];
